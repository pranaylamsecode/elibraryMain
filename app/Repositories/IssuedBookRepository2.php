 ...
 */
class IssuedBookRepository2 extends BaseRepository2 implements IssuedBookRepositoryInterface2
{
    /** @var BookItemRepository */
    private $bookItemRepo;

    public function __construct(Application $app, BookItemRepository2 $bookItemRepository)
    {
    ...
    public function all($search = [], $skip = null, $limit = null, $columns = ['*'])
    {
        ...
        $archived = false;
        if (isset($search['search']) && $search['search'] == IssuedBook2::STATUS_ARCHIVED) {
            unset($search['search']);
            $archived = true;
        }

        if (! empty($search['search']) && in_array($search['search'], IssuedBook2::STATUS_IN_STRING)) {
            $search['status'] = IssuedBook2::getStatusFromString($search['search']);
            unset($search['search']);
        }
        ...
        $overdueAndReserveDue = IssuedBook2::with(['bookItem.book', 'member']);
        ...
        // book issued filter
        $issuedFilter = (isset($search['status']) && $search['status'] == IssuedBook2::STATUS_ISSUED) ? true : false;
        ...
        $reservedFilter = (isset($search['status']) && $search['status'] == IssuedBook2::STATUS_RESERVED) ? true : false;
        ...
        $deletedFilter = (isset($search['status']) && $search['status'] == IssuedBook2::STATUS_ARCHIVED) ? true : false;
        ...
        $returnedFilter = (isset($search['status']) && $search['status'] == IssuedBook2::STATUS_RETURNED) ? true : false;
    ...
    public function store($input)
    {
        ...
        $issueBooked = IssuedBook2::create($input);
    ...
    {
        $issueBook = IssuedBook2::ofBookItem($input['book_item_id'])
            ->where('status', '!=', IssuedBook2::STATUS_RETURNED)
            ->exists();
    ...
    {
        $issuedBooksCount = IssuedBook2::ofMember($input['member_id'])->issued()->count();
        ...
        $input = [
            ...
            'status'          => IssuedBook2::STATUS_ISSUED,
            'issuer_id'       => Auth::id(),
        ];

        if (! empty($issueBook)) {
            if ($issueBook->status == IssuedBook2::STATUS_RESERVED && $issueBook->member_id != $input['member_id']) {
            ...
            if ($issueBook->status == IssuedBook2::STATUS_ISSUED) {
                throw new UnprocessableEntityHttpException('Book is already issued.');
            }

            $isUpdate = ($issueBook->status == IssuedBook2::STATUS_UN_RESERVED && $issueBook->member_id == $input['member_id']) ? true : false;
            if ($isUpdate || $issueBook->member_id == $input['member_id']) {
                $issueBook->update($input);
                $bookItem->update(['status' => BookItem2::STATUS_NOT_AVAILABLE]);
    ...
    {
        $reserveBooksCount = IssuedBook2::ofMember($input['member_id'])->reserve()->count();
        ...
        /** @var BookItem $bookItem */
        $bookItem = BookItem2::findOrFail($input['book_item_id']);
        if ($bookItem->status == BookItem2::STATUS_NOT_AVAILABLE) {
        ...
        if (! empty($issueBook) && $issueBook->status == IssuedBook2::STATUS_UN_RESERVED && $issueBook->member_id == $input['member_id']) {
            $issueBook->update($input);
        } else {
            $issueBook = IssuedBook2::create($input);
        }
    ...
    public function returnBook($input)
    {
        ...
        $returnedBook = IssuedBook2::ofBookItem($input['book_item_id'])
            ->ofMember($input['member_id'])->whereStatus(IssuedBook2::STATUS_RETURNED)
            ->first();
...
//        }
        ...
        $issueBook->update([
            ...
            'note'        => ! empty($input['note']) ? $input['note'] : null,
            'status'      => IssuedBook2::STATUS_RETURNED,
            'returner_id' => Auth::id(),
        ]);

        $bookItem->update(['status' => BookItem2::STATUS_AVAILABLE]);
    ...
    public function updateIssuedBookStatus($input)
    {
        if (! in_array($input['status'], [
            IssuedBook2::STATUS_DAMAGED,
            IssuedBook2::STATUS_LOST,
        ])) {
        ...
        if ($input['status'] == IssuedBook2::STATUS_LOST) {
            $input['status'] = BookItem2::STATUS_LOST;
        }

        if ($input['status'] == IssuedBook2::STATUS_DAMAGED) {
            $input['status'] = BookItem2::STATUS_DAMAGED;
        }
    ...
    {
        $unReservedBook = IssuedBook2::ofBookItem($bookItem->id)->ofMember($input['member_id'])
            ->whereStatus(IssuedBook2::STATUS_UN_RESERVED)
            ->first();
        ...
        /** @var IssuedBook $issueBook */
        $issueBook = IssuedBook2::ofBookItem($bookItem->id)
            ->where('status', IssuedBook2::STATUS_RESERVED)
            ->firstOrFail();
        ...
        $issueBook->update(['status' => IssuedBook2::STATUS_UN_RESERVED]);
        $bookItem->update(['status' => BookItem2::STATUS_AVAILABLE]);
    ...
    {
        $query = IssuedBook2::whereStatus(IssuedBook2::STATUS_ISSUED);
        if (! empty($startDate) && ! empty($endDate)) {
