...
use App\Http\Controllers\ResourceCategory;
use App\Http\Controllers\ResourceController;
use App\Http\Controllers\SiteVisitCounter;
...
Route::group(['middleware' => ['auth:sanctum']], function () {
    ...
    Route::post('transfers-return/{transfer_id}', [TransferAPIController::class, 'transferReturn']);
...
Route::get('/site-visits', [SiteVisitCounter::class, 'index']);
Route::post('/site-visits', [SiteVisitCounter::class, 'addCount']);
Route::resource('/resources', ResourceController::class);
Route::resource('/resources-category', ResourceCategory::class);
Route::get('/resource-export/{id}', function (Request $request) {
    $filename = ResourceModel::where('id', $request->id)->get()->toArray();
    /*  $data = Excel::load('file.csv', false, 'ISO-8859-1'); */
    $path = Storage::disk(config('filesystems.default'))->url("/Resources/" . $filename[0]['file_content']);
    return response()->json([
        'data' => $path, 'message' => 'Members exported successfully'
    ]);
});
