...
class IssuedBookRepository extends BaseRepository implements IssuedBookRepositoryInterface
{
    ...
    {
        $orderBy = null;
        if (!empty($search['order_by']) && in_array(
            $search['order_by'],
        ...
        $overdueAndReserveDue = IssuedBook::with(['bookItem.book', 'member']);
        ...
        // reserve_due filter
        $query->when(
            !empty($search['start_date']) && !empty($search['end_date']) && $reserveDueFilter,
        ...
        // overdue book filter
        $query->when(
            !empty($search['start_date']) && !empty($search['end_date']) && $overdueFilter,
            function (Builder $query) use ($search) {
        ...
        $query->when(
            !empty($search['start_date']) && !empty($search['end_date']) && $issuedFilter,
            function (Builder $query) use ($search) {
        ...
        $query->when(
            !empty($search['start_date']) && !empty($search['end_date']) && $reservedFilter,
            function (Builder $query) use ($search) {
                $query->whereDate('reserve_date', '>=', $search['start_date'])
                    ->whereDate('reserve_date', '<=', $search['end_date']);
            }
        ...
        $query->when(
            !empty($search['start_date']) && !empty($search['end_date']) && $deletedFilter,
            function (Builder $query) use ($search) {
        ...
        $query->when(
            !empty($search['start_date']) && !empty($search['end_date']) && $returnedFilter,
            function (Builder $query) use ($search) {
    ...
    public function reserveBook($input)
    {
        ...
        if ($bookItem->status == BookItem::STATUS_NOT_AVAILABLE) {
            if (!empty($issueBook) && $issueBook->note != $input['note'] && !empty($input['note'])) {
                $issueBook->update(['note' => $input['note']]);
