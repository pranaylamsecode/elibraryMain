...
class DashboardAPIController extends AppBaseController
{
    ...
    public function getPurchaseSalesCounts(): JsonResponse
    {
        ...
        $data['today_sales'] = Sale::where('date', $today)->sum('grand_total');
        $data['today_purchases'] = QuotationToPurchases::where('date', $today)->sum('grand_total');
        $data['today_sale_return'] = SaleReturn::where('date', $today)->sum('grand_total');
        $data['today_purchase_return'] = PurchaseReturn::where('date', $today)->sum('grand_total');
        $data['today_sales_received_count'] = SalesPayment::where('payment_date', $today)->sum('amount');
        $data['today_expense_count'] = Expense::where('date', $today)->sum('amount');
    ...
    public function getAllPurchaseSalesCounts(): JsonResponse
    {
        ...
        $data['all_sales_count'] = Sale::sum('grand_total');
        $data['all_purchases_count'] = QuotationToPurchases::sum('grand_total');
        $data['all_sale_return_count'] = SaleReturn::sum('grand_total');
        $data['all_purchase_return_count'] = PurchaseReturn::sum('grand_total');
        $data['all_sales_received_count'] = SalesPayment::sum('amount');
        $data['all_expense_count'] = Expense::sum('amount');
    ...
    public function getWeekSalePurchases(): JsonResponse
    {
        ...
        $sales = Expense::whereBetween('date', [$day['days'][0], $day['days'][6]])
            ->orderBy('date', 'desc')
            ->groupBy('date')
            ->get([
        ...
        $purchases = QuotationToPurchases::whereBetween('date', [$day['days'][0], $day['days'][6]])
            ->orderBy('date', 'desc')
            ->groupBy('date')
            ->get([
                DB::raw('DATE_FORMAT(date,"%Y-%m-%d") as week'),
    ...
    public function getTopCustomer(): JsonResponse
    {
        ...
        foreach ($topCustomers as $topCustomer) {
            ...
            $data['grand_total'][] = $topCustomer->grand_total;
        }
