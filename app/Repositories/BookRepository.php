    ...
    public function all($search = [], $skip = null, $limit = null, $columns = ['*'])
    {
        ...
        $query = $query->with(['genres', 'authors','items', 'items.publisher', 'items.language']);
         ...
         if(!empty($search['genre'])){
            ...
            $genre_result = Genre::select('id', 'name')
         ...
         if(!empty($search['publisher'])){
            ...
            $publisher_result = Publisher::select('id', 'name')
    ...
    public function validateItems($items)
    {
        foreach ($items as $item) {
            ...
            if (isset($item['book_code'])) {
                ...
                if ($bookItem->exists()) {
                    throw new UnprocessableEntityHttpException('Given book code ' . $item['book_code'] . ' is already exist.');
                }
            }
        }
